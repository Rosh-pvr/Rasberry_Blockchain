...
with open('blockchain_data.csv', 'w', newline='') as file:
    ...
    writer.writerow(["ID", "Sensor", "Priority", "Status"])
    
    for i in range(count):
        data = contract.functions.requests(i).call()
        writer.writerow([i, data[1], data[2], data[3]])
        if i % 50 == 0: print(f"Exported {i} records...")

print("Export Complete: blockchain_data.csv")
